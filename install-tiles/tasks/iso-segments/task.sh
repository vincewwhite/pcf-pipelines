...
additional_properties_config=$(jq-n \
...
properties_config=$(jq-s add additional_properties_config.json common_properties_config.json)
...
om-linux -t https://$OPS_MGR_HOST -u $OPS_MGR_USR -p $OPS_MGR_PWD -k configure-product -n $PRODUCT_IDENTIFIER -p "$properties_config" -pn "$network_config" -pr "$resources_config"
